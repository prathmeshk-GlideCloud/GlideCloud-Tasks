# 📘 GlideCloud Internship Tasks

## 👤 Intern Details

- **Name:** Prathmesh Kapde  
- **Role:** AI Engineer Intern  
- **Organization:** GlideCloud Solutions  
- **Internship Period:** January 2026 – Present  

---

## 📂 Repository Purpose

This repository contains **daily and task-based work** completed as part of the GlideCloud internship.

Each task is organized **by date**, with its own folder and internal documentation where required.

### 🎯 Objectives
- Maintain a clear record of daily progress  
- Ensure reproducibility of tasks  
- Follow professional repository organization standards  

---
🗓 Task Log

| Date        | Task                      | Tech Stack               | Status      |
| ----------- | ------------------------- | ------------------------ | ----------- |
| 06-Jan-2026 | FastAPI CRUD with MongoDB | FastAPI, MongoDB, Pytest | ✅ Completed |
| 07-Jan-2026 | FastAPI VectorDB          | FastAPI, Ollama, ChromaDB | ✅ Completed |
| 08-Jan-2026 | FastAPI Vector Searching  | FastAPI, Ollama, ChromaDB | ✅ Completed |
| 09-16 Jan 26 | Capstone Project         | Google Maps API, RAG | ✅ Completed |

